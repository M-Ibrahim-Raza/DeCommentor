"""
Python Comment, Docstring, and Type Hint Remover
    - Removes comments, docstrings, inline comments, and type hints from Python files.
"""
...
class DeCommentor(ast.NodeTransformer):
    """AST transformer to remove docstrings and type hints from Python code."""
    ...
    def visit_AnnAssign(self, node):
        # Convert annotated assignments (x: int = 5) to regular assignments (x = 5)
        if node.value is not None:
            # Create a regular assignment without the type annotation
            new_node = ast.Assign(
                targets=[node.target],
                value=node.value,
                lineno=node.lineno,
...
def remove_comments_and_docstrings(source_code: str) -> str:
    """
    Remove comments, docstrings, type hints, and inline comments from Python source code.

    Args:
        source_code: Python source code as string

    Returns:
        Cleaned source code without comments, docstrings, and type hints
    """
    try:
        # Parse the source code into AST
        tree = ast.parse(source_code)

        # Remove docstrings and type hints using AST transformer
        transformer = DeCommentor()
        tree = transformer.visit(tree)

        # Remove None nodes (from annotations without values)
                ...
                while i < len(line):
                    ...
                    if i + 2 < len(line) and line[i : i + 3] in ('"""', "'''"):
                        if not in_string:
                            ...
                            i += 3
                            continue
                        elif triple_quote and line[i : i + 3] == quote_char:
                            in_string = False
                            triple_quote = False
                            quote_char = None
                            i += 3
...
def main():
    parser = argparse.ArgumentParser(
        description="Remove comments, docstrings, type hints, and inline comments from Python files.",
...
Examples:
  ...
  python script.py project/ --ignore tests venv __pycache__
    ...
    parser.add_argument(
        "path", type=str, help="Path to Python file or directory to process"
    )
